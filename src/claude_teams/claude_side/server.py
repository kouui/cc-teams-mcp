...
from claude_teams.claude_side.spawner import discover_harness_binary, kill_tmux_pane, spawn_external, use_tmux_windows
...
async def app_lifespan(server):
    ...
    if not codex_binary:
        raise FileNotFoundError(
            "No external agent binary found on PATH. Install Codex CLI ('codex') or ensure it is in your PATH."
        )
...
def spawn_external_agent(
    ...
    via inbox, and begins working autonomously. Names must be unique within
    the team. cwd must be an absolute path.
...
def shutdown_external_agent(team_name: str, agent_name: str, ctx: Context) -> dict:
    ...
    removing it from team config, and resetting its tasks."""
